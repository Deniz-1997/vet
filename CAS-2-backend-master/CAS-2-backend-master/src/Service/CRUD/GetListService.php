...
class GetListService extends AbstractService
{
     ...
     * @param PsrCashReader $annotationReader
    ...
    protected function init(Request $request, $objectName)
    {
        ...
        $this->fields = json_decode($request->get('fields', '{}'), true);
     ...
     * @return array
     */
    public function getFieldsOperator(?array &$filters, &$operators = []): array
    {
        if (!$filters) {
            $filters = $this->filter;
        }
    ...
    public function getList(Request $request, string $objectName, array $context = [], bool $usageDoctrinePaginator = false): ApiResponseInterface
    {
        ...
        switch ($request->get('download')) {
            ...
            default:
                ...
                //                if ($eventResponse = $globalGetListEvent->getResponse()) {
                //                    $response = array_merge($response, $eventResponse);
                //                }
        ...
        foreach ($arFilter as $column => $filterValue) {
            ...
            if ($this->isEmmedable($this->metadata, $column, $filterValue)) {
                ...
                if (isset($this->metadata->fieldMappings[$column])) {
                    ...
                    if (in_array($type, ['json', 'jsonb', 'json_array'])) {
                ...
                if ($filterValueIsArray === true && isset($this->metadata->associationMappings[$columnWithoutType])) {
                    ...
                    if (is_string($filterValue)) {
                        try {
                            $isDateCondition = $this->createFilterDateTime($type, $filterValue, $columnWithoutType, $arExpressions);
                            if ($isDateCondition) {
                                continue;
                            }
    ...
    protected function getCondition(string $columnWithoutType, $filterValue, $operator = null, $isIgnoreLike = false): ?Comparison
    {
        ...
        $likeAutocomplete = getenv('FILTER_LIKE_AUTOCOMPLETE') == 1;
        if (is_string($filterValue)) {
                ...
                foreach ($columns as $col => $value) {
                    if (isset($value['column'])) {
                        ...
                        if (isset($metadataRelation->associationMappings[$rel])) {
                            ...
                            if ($expr = $this->createFilterDateTime($op, $value['value'], $value['column'], $arExpressions, true)) {
                                $criteria = Criteria::create();
     ...
     * != 18.07.2017: <18.07.2017 00:00:00 - >18.07.2017 23:59:59
     */
    private function createFilterDateTime($operator = '=', string $filterValue, string $columnWithoutType, array &$arExpressions, $returnExpr = false)
    {
