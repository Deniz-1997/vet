 ...
 */
final class Version20220516144714 extends AbstractMigration
{
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->abortIf($this->connection->getDatabasePlatform()->getName() !== 'postgresql', 'Migration can only be executed safely on \'postgresql\'.');
        $this->addSql("DELETE FROM action.action WHERE id = (SELECT id FROM action.action WHERE url='/admin/maintenance');");
        $this->addSql("DELETE FROM action_action_group WHERE action_id=(SELECT id FROM action.action WHERE code='maintenance')");
