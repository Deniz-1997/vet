...
class SecretStorageProvider extends VaultClient
{
    ...
    public function findEncodingUserKey(): ?string
    {
        ...
        $cacheKey = md5(date('Y:m') . md5(getenv('VAULT_TOKEN')));

        if (!$cache->hasItem($cacheKey)) {

            Tracer::getInstance()->createSpan('vault', 'Vault FindEncodingUserKey', 'CLIENT', ['endpoint' => getenv('VAULT_SERVER')]);
            $secretPath = getenv('VAULT_SECRET_PATH') ? getenv('VAULT_SECRET_PATH') : 'secret/paseto';
