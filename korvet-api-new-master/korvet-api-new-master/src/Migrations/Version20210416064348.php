 ...
 */
final class Version20210416064348 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql("DROP FUNCTION IF EXISTS update_items_count()");
        ...
        $this->addSql("CREATE FUNCTION public.update_items_count() RETURNS trigger LANGUAGE 'plpgsql' COST 100 VOLATILE NOT LEAKPROOF AS \$BODY\$
                         BEGIN
                             EXECUTE 'update action.action set items_count = (select count(*) from '||TG_TABLE_SCHEMA||'.'||TG_TABLE_NAME||' where deleted=false) where deleted=false and url=$1' USING TG_ARGV[0];
                             RETURN NULL;
                         END; \$BODY\$");
        ...
        $this->addSql("CREATE TRIGGER reference_category_nomenclature_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_category_nomenclature FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-category-nomenclature');");
        $this->addSql("CREATE TRIGGER reference_countries_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_countries FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-countries');");
        $this->addSql("CREATE TRIGGER reference_manufacturer_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_manufacturer FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-manufacturer');");
        $this->addSql("CREATE TRIGGER reference_measurement_units_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_measurement_units FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-measurement-units');");
        $this->addSql("CREATE TRIGGER reference_nomenclature_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_nomenclature FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-nomenclature');");
        ...
        $this->addSql("CREATE TRIGGER reference_release_form_items_count AFTER INSERT OR DELETE OR UPDATE ON reference.reference_release_form FOR EACH STATEMENT EXECUTE PROCEDURE update_items_count('/store/reference-release-form');");
    }
