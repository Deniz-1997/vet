...
class ApiAuthService extends ApiBaseConnector
{
    /**
    ...
    public function getAuthToken(bool $refresh = false): ?string
    {
        if ($refresh == false) {
            /** @var ApiToken */
            $token = $this->apiTokenRepository->findOneBy(['name' => 'SOCKET'], ['id' => 'DESC']);
            if ($token != null) {
    ...
    public function refreshAuthToken(): ?string
    {
        ...
        if ($accessToken == null) {
            throw new ApiException('Не удалось авторизоваться на сокет сервере', 'NOT_AUTHORIZED', null, 400);
        }
