...
abstract class PrintReport
{
    ...
    protected function saveFile($appKernel, $spreadsheet): string
    {
        ...
        switch ($this->reportType) {
            case 'shiftReport':
                $reportName = "Отчет за смену";
                break;
            case 'revenueReport':
                $reportName = "Отчет по выручке";
                break;
            case 'warehouseStatement':
                $reportName = "Ведомость по товарам на складах";
                break;
            case 'cullingReport':
                $reportName = "Отчет по отлову животных";
                break;
            case 'totalRevenueReport':
                $reportName = "Расшифровка по выручке";
                break;
        }
