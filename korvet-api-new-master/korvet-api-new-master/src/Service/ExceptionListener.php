...
class ExceptionListener implements DefaultListenerExceptionInterface
{
     ...
     */
    public function __construct(ApiResponse $response, LoggerInterface $logger, array $basePath, string $env, TranslatorInterface $translator, 
    ...
    public function onKernelException(ExceptionEvent $event): void
    {
        ...
        if ($this->isProd()) {
            ...
            if ($requestUrl != null) {
                $message .= "*RequestUrl*:" . $requestUrl . "%0A%0A";
            }
            $server = $_SERVER["REMOTE_ADDR"];
            if ($server != null) {
                $message .= "*RemoteIp*:" . $server . "%0A%0A";
            }
            $message .= "*Trace*: %0A" . $this->convertTrace(preg_replace('/#\d+/', '', $ex->getTraceAsString()), $message) . "%0A%0A";
            $this->telegram
                ->setMarkDown(true)
                ->setChannel(Env::getenv('TELEGRAM_EXCEPTION_CHAT_ID'))
