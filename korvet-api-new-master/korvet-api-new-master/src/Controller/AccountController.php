...
class AccountController extends AbstractController
{
     ...
     */
    public function __construct(LoggerInterface $logger, SendEmailService $sendEmailService, EntityManagerInterface $entityManager, SerializerInterface $serializer)
    {
        $this->logger = $logger;
     ...
     * @return Response
     */
    public function accountAction(BaseResponse $response, TokenStorageInterface $tokenStorage, AccessTokenRepository $accessTokenRepository)
    {
        $roles = array_map(function ($role) {
            return $role;
        }, $tokenStorage->getToken()->getRoleNames());
     ...
     * @throws ApiException
     */
    public function changePassword(TokenStorageInterface $tokenStorage, UserPasswordHasherInterface $passwordEncoder, UserRepository $userRepository, Request $request, BaseResponse $response): Response
    {
        /** @var User $user */
    ...
    public function startPasswordRecovery(Request $request, TranslatorInterface $translator, BaseResponse $response, UserRepository $userRepository)
    {
        ...
        $codeLength = ($isEmail) ? Env::getenv('PASSWORD_CONFIRMATION_CODE_EMAIL_LENGTH') : Env::getenv('PASSWORD_CONFIRMATION_CODE_SMS_LENGTH');
        ...
        if ($isEmail) {
            ...
            $email
                ...
                ->setEmailFrom(Env::getenv('PASSWORD_CONFIRMATION_EMAIL_FROM'))
     ...
     *
     */
    public function processChangePassword(Request $request, TranslatorInterface $translator, UserPasswordHasherInterface $passwordEncoder, UserRepository $userRepository, BaseResponse $response)
    {
        ...
        $expireLifetime = intval(Env::getenv('PASSWORD_CONFIRMATION_CODE_LIFETIME'));
        ...
        $newPassword = $passwordEncoder->hashPassword($user, $requestPasswordRecovery->newPassword);
