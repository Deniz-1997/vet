...
;(function () {
  ...
  function check_browser () {
    const ua = navigator.userAgent; let tem; let M = ua.match(/(opera|yabrowser|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || []
  ...
  var ru_cryptopro_npcades_10_native_bridge = {
    ...
    call: function call (functionName, args, callback) {
      ...
      const callbackId = hasCallback ? ru_cryptopro_npcades_10_native_bridge.callbacksCount++ : 0

      if (hasCallback) { ru_cryptopro_npcades_10_native_bridge.callbacks[callbackId] = callback }

      let iframe = document.createElement('IFRAME')
      const arrObjs = new Array('_CPNP_handle')
      try {
  ...
  function show_firefox_missing_extension_dialog () {
    if (!window.cadesplugin_skip_extension_install) {
      ...
      ovr.style = 'visibility: hidden; position: fixed; left: 0px; top: 0px; width:100%; height:100%; background-color: rgba(0,0,0,0.7); z-index: 998;'
      ovr.innerHTML = "<div id='cadesplugin_ovr_item' style='z-index: 1500; position:relative; width:400px; margin:100px auto; background-color:#fff; border:2px solid #000; padding:10px; text-align:center; opacity: 1'>" +
