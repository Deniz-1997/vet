<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <div class="title mb-4">
          <span>Сертификаты безопасности организации</span>
        </div>
          ...
          <template #[`item.actions`]="{ item }">
            <span class="d-flex align-items-center">
              ...
              <v-tooltip bottom>
                <template #activator="{ on, attrs }">
                  <span v-bind="attrs" v-on="on">
                    <img alt="" class="iconTable" src="/icons/deleteBasket.svg" @click="deleteItem(item.id)" />
                  </span>
                </template>
    ...
    <CertificateOrganizationCardModal v-model="isModalShow.card" :type="type" :details="details" @verify="onVerify" />
...
import CertificateOrganizationCardModal from '@/components/MyOrganization/Certificate/CertificateOrganizationCardModal.vue';

@Component({
  name: 'CertificateOrganizationPage',
  components: {
    DataTable,
    SearchComponent,
    CertificateManagementModal,
    CertificateOrganizationCardModal,
  },
  computed: {
    ...mapAccessFlags({
