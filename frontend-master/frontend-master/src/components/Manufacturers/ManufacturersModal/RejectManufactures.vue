  ...
  >
    ...
    <template #content>
      <UiForm :rules="rules" @validation="(v) => (isValid = v.isValid)" @submit="onSubmit">
        <v-row>
          <v-col cols="12">
            <div class="description">Отказ в регистрации товаропроизводителя {{ name }}</div>
          </v-col>
          <v-col cols="12">
...
export default class RejectManufacturers extends Vue {
  ...
  async getRejectList() {
    const { data } = await this.$service.manufacturer.getRejectList();
