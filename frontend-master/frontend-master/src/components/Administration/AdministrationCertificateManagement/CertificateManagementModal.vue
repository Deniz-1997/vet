<template>
  <DialogComponent v-model="isModalOpen" width="640" :persistent="isLoading" hide-actions>
    <template #title>
...
@Component({
  name: 'CertificateManagementModal',
  components: { DialogComponent, DefaultButton },
})
export default class CertificateManagementModal extends Mixins(Modal) {
  @Prop({ type: [Number, Array] }) readonly id!: number | number[];
  @Prop({ type: String, default: 'Загрузить сертификат' }) readonly title!: string;
  ...
  get rules() {
    const { maxSize, types, extensions } = this.$config.certificate;
    return {
      file: [
        'required',
        {
  ...
  get accepted() {
    if (!this.accept || !this.accept.length) {
      return this.$config.certificate.types;
    }
  ...
  async onSubmit() {
    ...
    try {
      const { data } = await this.service.create(this.form.file, this.onUploadProgress);
