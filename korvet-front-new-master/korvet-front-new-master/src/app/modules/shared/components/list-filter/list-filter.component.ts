...
export class ListFilterComponent implements OnInit, AfterContentInit {
  ...
  placeholderClinic = 'Ветеринарная станция по г.о Зарайск и Серебряные пруды';
  placeholderProfession = 'Хирург';

  mutableSearchOption: SearchOption[] = [
    {type: CrudType.Appointment, name: 'Приемы', url: '/'},
    {type: CrudType.Leaving, name: 'Выезды', url: '/leaving'},
    {type: CrudType.Owner, name: 'Владельцы', url: '/owners'},
    {type: CrudType.Pet, name: 'Животные', url: '/pets'},
    {type: CrudType.FormTemplate, name: 'Шаблоны', url: '/reference/form-template'}
  ];
  ...
  submit(): void {
    if (this.formGroup.valid) {
      ...
      if (this.mutableSearch && this.type !== model.mutableSearch.type) {
        ...
        const currentOption: SearchOption[] = this.mutableSearchOption.filter(item => item.type === model.mutableSearch.type);
        this.router.navigate([currentOption[0].url]).then();

      } else {
        if (this.type === CrudType.Pet && model.filter['appointments']) {
          if (model.filter['appointments']['>=date']) {
            model.filter['appointments']['>=date'] = this.convertAppointmentsDate(model.filter['appointments']['>=date']);
          }
          if (model.filter['appointments']['<=date']) {
            model.filter['appointments']['<=date'] = this.convertAppointmentsDate(model.filter['appointments']['<=date']);
          }
  ...
  convertAppointmentsDate(filter_data) {
    if ((filter_data as string).indexOf("'") != -1)
      return filter_data;
    const date = (filter_data as string).split('.');
    if (date.length === 3) {
