...
export class EditComponent extends ReferenceItemModels {
  protected listNavigate = ['admin', 'references', 'pet-aggressive-type'];
  protected titleName = 'Степень агрессивности животных';
  ...
  constructor(
    ...
    super(CrudType.ReferencePetAggressiveType, ReferencePetAggressiveTypeModel, data.id, data.openDialog);
  }

  protected setModel() {
    this.formGroup = new FormGroup({
      name: new FormControl(this.item.name, [Validators.required]),
      level: new FormControl(this.item.level, [Validators.required, Validators.max(10)]),
    });
  }
