...
import {LeavingModel, LeavingProductItem} from '../../../../../models/leaving/leaving.models';
...
@Component({
  templateUrl: './leaving-edit.component.html',
  styleUrls: ['./leaving-edit.component.css']
})
export class LeavingEditComponent implements OnInit, OnDestroy {
  crudType = CrudType;
  ...
  public model: LeavingModel = new LeavingModel();
  previousLeaving$: Observable<LeavingModel[]>;
  ...
  constructor(
    ...
    private changeLeavingStatus: LeavingChangeStatusService,
    ...
    this.previousLeaving$ = this.store.pipe(select(getCrudModelMatches, {type: CrudType.Leaving}));
  ...
  setModel() {
    ...
    if (this.model.pet) {
      ...
      this.getRepeatLeaving(this.model.pet.id);
    }
    ...
    try {
      this.formGroup = new FormGroup({
        ...
        temperature: new FormControl({
          value: (this.model.leavingTemperature && !this.model.temperatureNotMeasured) ? this.model.leavingTemperature.temperature.value : null,
          disabled: this.model.temperatureNotMeasured
        }),
        temperature_check: new FormControl(this.model.temperatureNotMeasured),
        weight: new FormControl({
          value: (this.model.leavingWeight && !this.model.weightNotMeasured) ? this.model.leavingWeight.weight.value / 1000 : null,
    ...
    this.formGroup.get('type.code').valueChanges
      ...
      .subscribe((res: string) => {
        if (res === 'SECONDARY') {
          ...
          if (petId) {
            this.getRepeatLeaving(petId, (item) => {
    ...
    this.formGroup.get('data').valueChanges
      ...
      .subscribe(() => {
        this.brdSrv.replaceLabelByIndex(this.getBrLeaving(), 2);
      });
  ...
  ngOnInit() {
    ...
    if (id) {
      this.params.id = id;
      if (!this.leavingPermission.getLeaving()) {
        ...
        this.store.dispatch(new LoadGetListAction({
          ...
          onSuccess: leaving => {
            this.model = new LeavingModel(leaving.response);
        ...
        this.store.pipe(
          select(getCrudModelStoreId, {type: CrudType.Leaving, params: id}),
          filter(leaving => !!leaving),
          takeUntil(this.destroy$),
        ).subscribe(leaving => {
          if (leaving && leaving.id) {
            this.model = leaving;
            if (leaving && leaving.owner) {
              this.ownerId = leaving.owner.id;
              this.owner = leaving.owner;
        ...
        this.store.dispatch(new LoadGetAction({type: CrudType.Leaving, params: id}));
      }
    } else {
      this.setModel();
    }
    if (code !== null) {
      this.changeLeavingStatus.changeLeavingStatusCode(code, this.model);
    }
  ...
  submit(): void {
    ...
    if (this.formGroup.valid) {
      ...
      if (!this.formGroup.controls['temperature_check'].value || this.model.leavingTemperature) {
        model.leavingTemperature = {
          'id': this.model.leavingTemperature ? this.model.leavingTemperature.id : 0,
          'leaving': {'id': model.id},
          'deleted': model.temperatureNotMeasured,
          'temperature': {
            'id': (this.model.leavingTemperature && this.model.leavingTemperature.temperature) ? this.model.leavingTemperature.temperature.id : 0,
      ...
      if (!this.formGroup.controls['weight_check'].value || this.model.leavingWeight) {
        model.leavingWeight = {
          'id': this.model.leavingWeight ? this.model.leavingWeight.id : 0,
          'leaving': {'id': model.id},
          'deleted': model.weightNotMeasured,
          'weight': {
            'id': (this.model.leavingWeight && this.model.leavingWeight.weight) ? this.model.leavingWeight.weight.id : 0,
            'date': this.model.date,
      ...
      this.loading$ = this.store.pipe(select(getCrudModelCreatePatchLoading, {type: this.type}));
      this.store.dispatch(new LoadPatchAction({
        type: this.type,
        params: model,
        onSuccess: (res) => {
          if (res.status === true && res.response && res.response.id) {
            this.router.navigate(['/leaving/', this.model.id]).then();
          }
        },
        onError: (err) => {
          if (err.errors.length && err.errors[0].stringCode === 'LEAVING_CHANGED') {
      ...
      if (this.formGroup.controls.productItems &&
        ...
        if (this.formGroup.controls.pet.value?.id === undefined) {
          this.notify.handleMessage('Добавьте владельца и животное', 'warning');
        } else {
  ...
  getBrLeaving(): string {
    return (this.formGroup.value.type && this.formGroup.value.type.code === 'SECONDARY' ? 'Повторный' : 'Первичный') + ' выезд от '
      + this.formGroup.value.data + ' "'
      + this.formGroup.value.time + ' "'
      + (this.formGroup.value.pet?.name === undefined ? '' : this.formGroup.value.pet.name) + '';
  }
  ...
  getOwnerId() {
    ...
    if (this.ownerId !== undefined) {
      this.store.dispatch(new LoadGetListAction({
        type: CrudType.Owner,
        params: {
          filter: {id: this.ownerId}
        },
        onSuccess: (res) => {
          this.owner = res.response.items[0];
          const id = this.owner.pets[0]['pet']['id'];
          const name = this.owner.pets[0]['pet']['name'];
          this.formGroup.get('pet').setValue({id: id, name: name});
          this.idPets = id;
          this.petName = name;
          this.brdSrv.replaceLabelByIndex(this.getBrLeaving(), 2);
        }
      ...
      this.formGroup.get('pet').setValue(null);
      this.idPets = this.formGroup.controls.pet.value?.id;
      this.petName = this.formGroup.controls.pet.value?.name;
    }
  }
  getPetId() {
    this.idPets = this.formGroup.controls.pet.value?.id;
