...
import {AddDialogComponent as AppointmentsAddDialogComponent} from '../../add-dialog/add-dialog.component';
import {Router} from '@angular/router';
import {DatePipe} from '@angular/common';
import {CrudType} from '../../../../../../common/crud-types';
import moment from 'moment';
import {CrudState} from 'src/app/api/api-connector/crud/crud-store.service';
import {LoadGetListAction, LoadAppendListAction, LoadPatchAction} from 'src/app/api/api-connector/crud/crud.actions';
...
export class ScheduleComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.loading$ = combineLatest([
      this.store.pipe(select(getCrudModelGetListLoading, {type: CrudType.ReferenceAppointmentStatus})),
  ...
  ngOnInit() {
    ...
    this.appointments$ = this.store.pipe(select(getCrudModelData, {type: this.type}));
  ...
  getApp() {
    ...
    for (const key of Object.keys(this.scheduleDay)) {
      this.scheduleDay[key].appointments = {};
    }
    ...
    this.store.dispatch(new LoadGetListAction({
      ...
      onSuccess: result => {
        if (result.response.items) {
          result.response.items.map(
            appointment => {
              if (!this.scheduleDay.hasOwnProperty(appointment.user.id)) {
                // console.log('ddd', appointment);
                this.scheduleDay[appointment.user.id] = {};
                this.scheduleDay[appointment.user.id].user = appointment.user;
                if (!this.scheduleDay[appointment.user.id].hasOwnProperty('appointments')) {
                  this.scheduleDay[appointment.user.id].appointments = {};
                }
              }

              const min = Math.round(appointment.date.split(' ')[1].split(':')[1] / 10) * 10;
              const time = appointment.date.split(' ')[1].split(':')[0] + ':' + (min !== 0 ? min : '00');

              this.scheduleDay[appointment.user.id].appointments[time] = {
  ...
  addAppointment(user, time, disabled) {
    if (!disabled) {
      const dialogRef = this.dialog.open(AppointmentsAddDialogComponent, {
  ...
  goToView(id, $event?) {
    ...
    this.router.navigate(['/appointments', id]).then();
  }
  ...
  scrollToTime() {
    ...
    if (today.getDate() === this.selectedDate.getDate() && today.getMonth() === this.selectedDate.getMonth()) {
      ...
      // const time = document.querySelector('.appointment:not(.disabled)')
      //   .parentElement
  ...
  drop(item) {
    ...
    if (item) {
      ...
      appointmentDate = this.date + ' ' + this.hour + ':' + this.step + ':' + '00';
